...
static int esp_hid_gap_request_low_latency(uint16_t conn_handle)
{
    struct ble_gap_upd_params params = {
        ...
        .itvl_max = BLE_GAP_CONN_ITVL_MS(7.5),
        .latency = 0,
...
nimble_hid_gap_event(struct ble_gap_event *event, void *arg)
{
    ...
    case BLE_GAP_EVENT_CONNECT:
        ...
        if (event->connect.status == 0) {
            esp_hid_gap_request_low_latency(event->connect.conn_handle);
        } else {
    ...
    case BLE_GAP_EVENT_CONN_UPDATE:
        ...
        if (event->conn_update.status == 0) {
            ...
            if (rc == 0) {
                ESP_LOGI(TAG, "connection updated: itvl=%.2f ms latency=%d",
            ...
            ESP_LOGW(TAG, "connection update rejected; status=%d",
                     event->conn_update.status);
        }
        return 0;
    ...
    case BLE_GAP_EVENT_ENC_CHANGE:
        ...
        assert(rc == 0);
        esp_hid_gap_request_low_latency(event->enc_change.conn_handle);
        ble_hid_task_start_up();
        return 0;
